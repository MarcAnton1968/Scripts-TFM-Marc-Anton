#################################################################################
# Monte Carlo method to calculate Multi-Species Indices (MSI) 2.3
...
#   - year
#   - index (base year has index 100; any year can be the base yeaR
#   - se (standard error of index; base year has SE zero)
################################################################################

set.seed(9478)
#setwd("C:/Marc/indicadors/LPI/recerca/Sensitivity_analisis/MSI")# the path for input and output files.
rdata <- read.table("species_trends_LPI.txt", header=TRUE) # name of input file.
jobname <- as.character ("LPI_CAT") # generic name for output files.
rdata$index <- as.numeric(rdata$index)
rdata$se <- as.numeric(rdata$se)
nsim <- 1000		    # number of Monte Carlo simulations.
SEbaseyear <- 2002 # desired year to set MSI to 1000 and SE to 0; usually the first year of the time series.
plotbaseyear <- 2002 # desired year to set to 100 in plots.
...
maxCV <- 10       # maximum allowed mean Coefficient of Variation (CV) of species indices (0.5 = 50%). Species with higher mean CV are excluded.
changepoint <- 2012 # compare trends before and after this year.
